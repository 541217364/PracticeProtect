...
@interface CustomWebController ()<WKUIDelegate,WKNavigationDelegate,WKScriptMessageHandler>
...
- (WKWebView *)wkWebView{
    if (!_wkWebView) {
       ...
       // [userContentController addScriptMessageHandler:self name:@"jsCallOC"];
        ...
        self.wkWebView = [[WKWebView alloc] initWithFrame:CGRectMake(0, HeightForNagivationBarAndStatusBar, SCREEN_WIDTH, SCREEN_HEIGHT -HeightForNagivationBarAndStatusBar - HOME_INDICATOR_HEIGHT) configuration:configuration];
        // 设置代理
...
- (UIProgressView* )progress {
    if (!_progress) {
        ...
        _progress.frame = CGRectMake(0, STATUS_BAR_HEIGHT, self.view.width, 4);
        _progress.autoresizingMask = UIViewAutoresizingFlexibleWidth|UIViewAutoresizingFlexibleBottomMargin;
//        _progress.tintColor = _loadingProgressColor ? _loadingProgressColor:TRMainColor;
//        _progress.backgroundColor = TRGrayColor;
...
- (void)loadRequest {
    if (![self.urlString hasPrefix:@"http"]) {
        //容错处理 不要忘记plist文件中设置http可访问 App Transport Security Settings
